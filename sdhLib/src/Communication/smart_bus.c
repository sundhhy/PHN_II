...
int SmBus_Set_cold_tmpr(IN uint8_t clt, OUT uint8_t *frame_buf, int buf_size)
{
	smBus_head_t	*s_head = (smBus_head_t *)frame_buf;
	uint8_t			*playload = frame_buf + sizeof(smBus_head_t);
	smBus_tail_t	*s_tail = (smBus_tail_t *)(playload +  6);
	uint8_t			sb_xor = 0;
	if(buf_size < 9)
		return ERR_MEM_LACK;
	
	s_head->cmd = SMBUS_CMD_SET_CLT;
	s_head->src_addr = SMBUS_SRC_ADDR;
	s_head->dst_addr = 0x50;
