...
#define SYSFLAG_CHN_ERR				1			//ÏòÍ¨µÀ°å¶ÁÈ¡Êý¾ÝÎÞ»Ø¸´Ê±£¬ÈÏÎªÍ¨µÀ°åÎ´Á¬½Ó
...
#define SYSFLAG_LOWSPACE			0x80		//´æ´¢ÀúÊ·Êý¾ÝµÄ¿Õ¼ä¼´½«ºÄ¾¡


#define STG_DEFAULT_ALARM			86400		//Ò»ÌìµÄÃëÊý
...
typedef struct {
	uint8_t		num_chn;		//¿ÉÒÔÔÚ³¬¼¶ÉèÖÃ½çÃæÖÐÉèÖÃ£¬µ«²»µÃ´óÓÚNUM_CHANNEL
	...
	uint8_t		communication_mode;			//ÒÇ±íÓëpcÁ¬½Ó£º Í¨Ñ¶£» ÒÇ±íÓë´òÓ¡»úÁ¬½Ó: ´òÓ¡   12B
	
	uint8_t		id;											// 1 - 63
	uint8_t		baud_idx;
	uint8_t		sys_flag;
	uint8_t		cold_end_way;				//0 Íâ²¿£¬ 1 Éè¶¨			16B
	
	uint8_t		CJC;								//Àä¶Ë²¹³¥ 0-99 ÎªÉè¶¨Ä£Ê½
	uint8_t		disable_modify_adjust_paramter;		//½ûÖ¹ÐÞ¸Äµ÷½Ú²ÎÊý
	uint8_t		show_chn_status;					//½ûÖ¹Í¨µÀ×´Ì¬ÏÔÊ¾
	uint8_t		enable_beep;											//°´¼üÉùÒôÔÊÐí			20B
	
	int 			baud_rate;
	uint32_t		storage_alarm;			//ÀúÊ·Êý¾Ý´æ´¢ÈÝÁ¿¿ìºÄ¾¡Ê±»á²úÉú¾¯¸æÐÅºÅ£¬Õâ¸öÖµ×÷Îª·§Öµ			28B
}system_conf_t;
